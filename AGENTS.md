...
`AGENTS.md` is the primary instruction file for this repository. `CLAUDE.md` is a
...
- Check `brand_assets/web/` first, then `brand_assets/originals/`, then `assets/web/`, then `assets/originals/`. Use real brand assets before placeholders.
...
- Port 8123 is also the journal's local origin. Use `tools/journal.ps1` / `tools/journal_server.py` when checking the live journal, and do not start a second server on that port. The Claude launch configuration for the site is only a static preview and does not provide `/sync`.
- The deploy bundle can be previewed on port 8124 with `python -m http.server 8124 --directory deploy` when Python is available. These commands replace the project-specific `.claude/launch.json` presets for agents outside Claude.
...
- Treat `AGENTS.md` as the primary instruction file. `CLAUDE.md` exists only to point Claude back here.
- After every completed stage of work (finishing a section, adding a feature, fixing a layout issue, optimizing assets), create a git commit and push to `main`. Do not batch multiple stages into one commit.
- Commit message must describe what was done and why, not just "update".
...
- Never skip the commit/push step at the end of a work session.

## Asset Layout

- Production files that can be served publicly live in the repository root, `brand_assets/web/`, and `assets/web/`.
- Optimized web media (WebP, compressed JPG/PNG, SVG) goes into `assets/web/`. Reference these paths from `index.html`.
- Heavy originals, RAW files, and source-only material go into `assets/originals/`. Never reference these directly from `index.html`.
- Web-ready brand files (logo SVG/PNG, palette swatches) go into `brand_assets/web/`.
- Source brand files (AI, PSD, original exports) go into `brand_assets/originals/`.
- Do not delete heavy originals unless the user explicitly asks. Move them to `originals/` instead.
- `assets/originals/` and `brand_assets/originals/` are excluded from deployment via `.vercelignore` and `.netlifyignore`.

## Asset Optimization (mandatory, do not skip)

Every time an asset is added to the project or used in the site, the agent must:

1. **Inspect the original** — check format, dimensions, and file size before using it.
2. **Convert and compress automatically** — do not ask permission, just do it:
   - Photos → WebP, quality 80–85, max width 1920px for hero / 800px for thumbnails.
   - Logos / icons → SVG preferred; if raster, PNG with `pngquant` or equivalent, max 200KB.
   - Strip all metadata (EXIF, color profiles) unless color accuracy is critical.
3. **Save the optimized file to `assets/web/` or `brand_assets/web/`** — never overwrite the original.
4. **Reference only the `web/` version in `index.html`** — never link to `originals/`.
5. **Report the before/after size** — e.g. `hero.jpg 4.2MB → hero.webp 180KB`.

Optimization targets (reject anything above these without a written reason):
- Hero image: ≤ 300KB
- Gallery / content images: ≤ 150KB each
- Logo (raster): ≤ 50KB
- Total page weight (HTML + CSS + JS + above-fold images): ≤ 1MB

Tools to use on this machine (Windows, Python available):
```
# Convert + compress with Pillow (already available or pip install Pillow)
python -c "
from PIL import Image
img = Image.open('assets/originals/hero.jpg')
img = img.convert('RGB')
img.thumbnail((1920, 1920))
img.save('assets/web/hero.webp', 'WEBP', quality=82, method=6)
"
# Check file sizes
(Get-Item assets/web/hero.webp).length / 1KB
```

If Python/Pillow is not available, use the `sharp` CLI (`npx sharp-cli`) or inline `<canvas>` downscale as a last resort. Never ship an unoptimized original.
...
- Always inspect `brand_assets/web/` before designing, then `brand_assets/originals/` if web versions are missing.
- If a logo exists in `brand_assets/web/`, use it. If only in `originals/`, use it but note it may need optimization.
- If a color palette, typography sample, style tile, or screenshot exists, derive the design from it.
- If assets exist in `assets/web/`, prefer them over remote placeholder media.
...
- Lazy-load non-critical images (`loading="lazy"` + explicit `width`/`height`).
- Preload the hero image: `<link rel="preload" as="image" href="assets/web/hero.webp">`.
- Serve fonts with `font-display: swap` and subset to used characters where possible.
...
- All images must be optimized before use — see **Asset Optimization** section above.
...
- Do not ignore `brand_assets/` or `assets/`. Always check both `web/` and `originals/` subfolders.
